...
// ============================================
// PRE-VALIDATE MIDDLEWARE 1: Auto-generate Purchase Order Number
// ============================================
purchaseOrderSchema.pre('validate', async function(next) {
  if (this.isNew && !this.purchaseOrderNumber) {
    try {
      ...
      if (lastPO && lastPO.purchaseOrderNumber) {
        const match = lastPO.purchaseOrderNumber.match(/PO-(\d+)/);
        if (match) {
          nextNumber = parseInt(match[1], 10) + 1;
        }
      }

      this.purchaseOrderNumber = `PO-${String(nextNumber).padStart(2, '0')}`;
      console.log('✅ Generated purchase order number:', this.purchaseOrderNumber);
    } catch (error) {
...
// ============================================
// PRE-SAVE MIDDLEWARE 2: Calculate purchase order totals
// ============================================
purchaseOrderSchema.pre('save', function(next) {
  if (this.lineItems && this.lineItems.length > 0) {
    console.log('🔄 Calculating purchase order totals...');
...
// ============================================
purchaseOrderSchema.methods.calculateLineItemAmounts = async function() {
